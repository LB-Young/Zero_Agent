...
def play_audio(pcm_file_path):
    if os.path.isfile(pcm_file_path):
        player = PCMPlayer(channels=1,          # 单声道
                        sample_width=2,        # 16位采样
                        sample_rate=16000)     # 16kHz采样率
        
        # 播放PCM文件
        pcm_file = pcm_file_path
        if os.path.exists(pcm_file):
            player.play_pcm_file(pcm_file)
    else:
        all_audio_files = os.listdir(pcm_file_path)
        sorted_files = sorted(all_audio_files, key=lambda x: int(x.split('.')[0]))
        for file in sorted_files:
            player = PCMPlayer(channels=1,          # 单声道
                            sample_width=2,        # 16位采样
                            sample_rate=16000)
            pcm_file = os.path.join(pcm_file_path, file)
            if os.path.exists(pcm_file):
                player.play_pcm_file(pcm_file)
